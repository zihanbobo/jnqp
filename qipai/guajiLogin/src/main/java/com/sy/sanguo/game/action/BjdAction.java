...
//                }
     ...
     * 修改群最大人数
     */
    public void updateGroupMaxCount() {
        try {
            Map<String, String> params = UrlParamUtil.getParameters(getRequest());
            LOGGER.info("BjdAction|updateGroupMaxCount|params|" + params);
            if (!BjdUtil.checkSign(params)) {
                response(-1, LangMsg.getMsg(LangMsg.code_1));
                return;
            }
            ...
            MessageBuilder msg = MessageBuilder.newInstance();
            msg.builderCodeMessage(0, LangMsg.getMsg(LangMsg.code_0));
            msg.builder("maxCount", maxCount);
            msg.builder("ret", ret);
            response(msg);
            LOGGER.info("BjdAction|updateGroupMaxCount|succ|" + maxCount + "|" + ret + "|" + params);
        } catch (Exception e) {
            response(2, LangMsg.getMsg(LangMsg.code_4));
            LOGGER.error("updateGroupMaxCount|error|" + e.getMessage(), e);
        }
    }
