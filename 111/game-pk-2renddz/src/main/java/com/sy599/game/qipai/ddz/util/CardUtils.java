...
//        ls.add(minboom);
    ...
    public static int getDpBeiShu(List<Integer> integers,int op_dipaijiabei) {
        ...
        if(loadCardColor(integers.get(0))==loadCardColor(integers.get(1)) && loadCardColor(integers.get(1))==loadCardColor(integers.get(2))){
            bs=3;//同花
            istonghua = true;
            //同花顺 X4
            int index = p2.get(0);
            if((index+1)==p2.get(1) && (index+2)==p2.get(2)){
                bs = 4;
            }
        }else {
        ...
        int index = p2.get(0);
        if((index+1)==p2.get(1) && (index+2)==p2.get(2) && !istonghua){
            //不是同花顺X3
            bs = 3;
        }
//        if(op_dipaijiabei>0 && bs!=1){
//            bs=bs*2;
//        }
        return bs;
    }
